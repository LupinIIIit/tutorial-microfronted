...
export class App implements OnInit {
  ...
  async ngOnInit() {
    try {
      ...
      // Imposta il titolo specifico per cart
      this.layoutComponentRef.instance.appTitle = 'TechStore - Carrello';
  ...
  private async showFallbackLayout() {
    try {
        ...
        <div style="min-height: 100vh; background: #f8fafc; padding: 2rem;">
          <div style="max-width: 1200px; margin: 0 auto;">
            <header style="background: #2563eb; color: white; padding: 1rem 2rem; border-radius: 8px; margin-bottom: 2rem;">
              <h1>TechStore - Carrello (Modalità Fallback)</h1>
            </header>
      ...
      // Carica e inserisci il component cart nel fallback
      const { CartComponent } = await import('./cart/cart.component');
      const contentArea = fallbackElement.querySelector('#fallback-content');
      if (contentArea) {
        const cartComponent =
          this.viewContainerRef.createComponent(CartComponent);
        contentArea.appendChild(cartComponent.location.nativeElement);
      }
    } catch (error) {
      ...
      document.body.innerHTML = `
        <div style="min-height: 100vh; display: flex; align-items: center; justify-content: center; background: #fee2e2; color: #dc2626;">
          <div style="text-align: center;">
            ...
            <p>Impossibile caricare il micro-frontend Cart</p>
